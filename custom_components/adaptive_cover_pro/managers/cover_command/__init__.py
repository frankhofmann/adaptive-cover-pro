...
class CoverCommandService:
    ...
    async def apply_position(
        ...
        *,
        ...
        #
        # For a NEW non-endpoint target (normal solar tracking moves) this gate
        # uses EXACT equality only — it is NOT a hysteresis band.  Movement
        # hysteresis (how big a move must be before we re-command) is owned solely
        # by _check_position_delta below, governed by the user's
        # CONF_DELTA_POSITION. Using the reconciliation tolerance for all targets
        # conflated the two concepts and suppressed legitimate small tracking
        # moves (issue #567).
        #
        ...
        if (
            ...
            # the endpoint-tolerance one. Always record `_plan.routed_target`
            # — the module's own SSOT definition of "the target" (routing.py's
            # docstring; `_prepare_service_call` records the identical value on
            # a real dispatch) — never substitute `_current`. This booking is
            # not diagnostics-only: it also makes the entity
            # reconciliation-eligible, so a later drift off the booked value
            # resends it on the next reconciliation pass exactly as it would
            # for any other target.
            #
            # The exception: arm 1's endpoint-tolerance sub-arm, where
